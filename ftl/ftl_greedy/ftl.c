...
UINT32 g_ftl_num_write = 0;    // # of write sectors
UINT32 g_ftl_num_read = 0;     // # of read sectors
UINT32 g_ftl_num_gc = 0;       // # of gc
UINT32 g_ftl_num_erase = 0;    // # of erased blocks
UINT32 g_ftl_num_copyback = 0; // # of copy-backed pages
...
static void sanity_check(void)
{
    UINT32 dram_requirement = RD_BUF_BYTES + WR_BUF_BYTES + COPY_BUF_BYTES + FTL_BUF_BYTES + HIL_BUF_BYTES + TEMP_BUF_BYTES + BAD_BLK_BMP_BYTES + PAGE_MAP_BYTES + VCOUNT_BYTES;
...
void ftl_read(UINT32 const lba, UINT32 const num_sectors)
{
    ...
    while (remain_sects != 0)
    {
        ...
        g_ftl_num_read += num_sectors_to_read;
...
void ftl_write(UINT32 const lba, UINT32 const num_sectors)
{
    ...
    while (remain_sects != 0)
    {
        ...
        g_ftl_num_write += num_sectors_to_write;
...
static void garbage_collection(UINT32 const bank)
{
    ...
    for (src_page = 0; src_page < (PAGES_PER_BLK - 1); src_page++)
    {
        ...
        g_ftl_num_copyback += SECTORS_PER_PAGE;
